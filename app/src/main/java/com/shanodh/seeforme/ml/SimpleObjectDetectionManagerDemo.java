...
/**
 * Simple YOLO11 Demo Manager with Batch Voice Output
 * Announces all detected labels at once
 */
public class SimpleObjectDetectionManagerDemo {
    private static final String TAG = "DemoDetect";
    ...
    public SimpleObjectDetectionManagerDemo(Context context) {
    ...
    private MappedByteBuffer loadModel(Context context) throws IOException {
        FileChannel fc = context.getAssets()
            .openFd(MODEL_PATH)
            .createInputStream()
            .getChannel();
    ...
    /**
     * Runs detection and announces unique labels via TTS in one batch
     */
    public List<SimpleDetection> detect(Bitmap bitmap) {
        List<SimpleDetection> results = new ArrayList<>();
        if (!initialized) {
        ...
        for (int i = 0; i < output[0][0].length; i++) {
            ...
            if (conf > CONFIDENCE_THRESHOLD) {
                int classIdx = i % COCO_CLASSES.length; // simple mapping for demo
                String label = COCO_CLASSES[classIdx];
                results.add(new SimpleDetection(label, conf));
                uniqueLabels.add(label);
            }
        }

        Log.i(TAG, "Detected " + results.size() + " items");
        announceBatch(uniqueLabels);
        return results;
    }

    private void announceBatch(Set<String> labels) {
        if (labels.isEmpty()) return;
        String msg = String.join(", ", labels) + " detected";
        mainHandler.post(() -> {
            if (tts != null) tts.speak(msg, TextToSpeech.QUEUE_FLUSH, null, "batch_detect");
            Log.i(TAG, "TTS Batch: " + msg);
        });
    }
    ...
    /**
     * Simple detection result for demo
     */
    public static class SimpleDetection {
        public final String label;
        public final float confidence;

        public SimpleDetection(String lbl, float conf) {
            this.label = lbl;
...
//     // Model Configuration - Optimized based on article insights
//     private static final String MODEL_PATH = "yolo11n_float32.tflite";
//     private static final float DETECTION_CONFIDENCE = 0.4f; // Higher threshold for quality detections
//     private static final float NMS_THRESHOLD = 0.45f; // Standard NMS
//     private static final float MIN_DETECTION_SIZE = 0.0015f; // Very permissive
    
//     // Timing Configuration
//     private static final long DETECTION_INTERVAL = 250; // Faster detection for real-time
//     private static final long ANNOUNCEMENT_COOLDOWN = 5000; // 5 second cooldown to prevent repetition
...
//     // State Management
//     private Map<String, Long> lastAnnouncementTime;
//     private Set<String> currentlyDetectedObjects; // Track what's currently visible
...
//         this.currentlyDetectedObjects = new HashSet<>();
...
//                 Log.d(TAG, "🔍 Detected " + filteredDetections.size() + " objects in " + processingTime + "ms");
                
//                 // Log detected objects
//                 if (!filteredDetections.isEmpty()) {
//                     StringBuilder objectList = new StringBuilder("📋 Objects: ");
//                     for (Detection d : filteredDetections) {
//                         objectList.append(d.getLabel()).append("(").append(String.format("%.2f", d.getConfidence())).append(") ");
//                     }
...
//             Log.d(TAG, String.format("🔧 Input tensor shape: [%d][%d][%d][%d]", 
...
//                 Log.e(TAG, "❌ Tensor shape mismatch: " + e.getMessage());
                
//                 // Try to get actual expected input shape
//                 int[] expectedShape = model.getInputTensor(0).shape();
//                 Log.e(TAG, "Expected input shape: " + Arrays.toString(expectedShape));
//                 Log.e(TAG, "Provided input shape: [" + input.length + "][" + input[0].length + "][" + input[0][0].length + "][" + input[0][0][0].length + "]");
                
//                 return detections; // Return empty list on error
//             }
...
//         Log.d(TAG, String.format("📐 Preprocessed image: %dx%dx%d", height, width, 3));
//         return input;
//     }
...
//                 Log.d(TAG, String.format("🔍 Candidate %d: %s conf=%.4f coords=[%.3f,%.3f,%.3f,%.3f]", 
//                     totalCandidates, 
//                     COCO_CLASSES[bestClassIdx],
//                     bestConfidence,
...
//                     Log.d(TAG, String.format("✅ Valid detection: %s conf=%.3f size=%dx%d", 
//                         className, bestConfidence, detectionWidth, detectionHeight));
//                 }
//             }
//         }
        
//         Log.d(TAG, "📊 Processed " + totalCandidates + " candidates, found " + validDetections + " valid detections");
        
//         return detections;
//     }

//        private void processAnnouncements(List<Detection> detections) {
//         // Simply announce every detected object with direction - NO MEMORY
//         for (Detection detection : detections) {
//             String objectName = detection.getLabel();
...
//         // If nothing detected, stay silent (no announcement)
//         if (detections.isEmpty()) {
//             Log.d(TAG, "🔇 No objects detected - staying silent");
//         }
//     } 
...
//                 if (calculateIoU(current, other) > NMS_THRESHOLD) {
//                     suppressed[j] = true;
//                 }
//             }
//         }
        
//         Log.d(TAG, "🎯 NMS: " + detections.size() + " → " + filtered.size() + " detections");
//         return filtered;
//     }
...
//     /**
//      * Generate simple announcements for detected objects - just announce with direction
//      */
//     private void processAnnouncements(List<Detection> detections) {
//         long currentTime = System.currentTimeMillis();
        
//         // Announce new or eligible objects
//         for (Detection detection : detections) {
//             String objectName = detection.getLabel();
//             Long lastTime = lastAnnouncementTime.get(objectName);
            
//             if (lastTime == null || (currentTime - lastTime) > ANNOUNCEMENT_COOLDOWN) {
//                 // Simple announcement with direction
//                 String direction = getDirection(detection);
//                 String message = objectName + " to the " + direction;
                
//                 speakMessage(message);
//                 lastAnnouncementTime.put(objectName, currentTime);
...
//         }
//         Log.d(TAG, "🧹 Detection state cleared - starting fresh");
//     }
...
//                ", Announced objects: " + lastAnnouncementTime.size();
//     }
